...
public class WordToHtml {
	...
	public static String convert(byte[] inputFile) throws Exception {
		InputStream input = new ByteArrayInputStream(inputFile);
		...
		if (pics != null) {
			for (int i = 0; i < pics.size(); i++) {
				...
				try {
					pic.writeImageContent(new FileOutputStream(wordImageFilePath() + pic.suggestFullFileName()));
...
//		FileUtils.writeStringToFile(new File(htmlFile), content, "utf-8");
	...
	public static String wordImageFilePath() {
		...
		return "D:/develop/upload/wordimage/" + sdf.format(new Date());
	}
	...
	public static String wordImgeWebPath() {
		DateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		return "D:/develop/wordimage/" + sdf.format(new Date()) + "/";
	}
