var userTable;	//用户列表
			...
			//查询
			function search() {
				var id="usernameSearch";
				var idval=$("#usernameSearch").val();
				checkText(id,idval);
...
//								}
		...
		*/
		...
		function testDing(){			
			$("#dataNotice input").val("");
			$("#dataNotice select").val(0);
			var noticeUser = "";
			var info = userTable.bootstrapTable("getSelections");
			if(info != 0) {
					id = info[0];
					if(info[0].active) {
		//				loadRole(getRole(), findUserRole(info[0].id));
						noticeUser= findNoticeUser(info[0].id);
	//					alert(noticeUser.userId);
						...
						if (noticeUser != "") {
							debugger
							$("#mobile").val(noticeUser.mobile);
							$("#userId").val(noticeUser.userId);
						}else {
							$("#mobile").val(info[0].mobilePhone);
							$("#userId").val("");
						}
						
						var dialog = getJqueryDialog();
				   		dialog.Container = $("#dataNotice");
				   		dialog.Title = "钉钉绑定";
				   		dialog.CloseOperation = "destroy";
				   		dialog.BeforeClose = function(){tipsRegionHintAddVerify($("#dataNotice"));};
						//dialog.Open = function(){tipsRegionHintRemoveVerify($("#dataNotice"));};
				   		var id = info[0].id;
				   		dialog.ButtonJSON = {
							"取消" : function() {
								$(this).dialog("close");
							...
							"确定" : function() {
										$.ajax({
											type : 'post',
											url : 'noticeUserAction!createRUserNotice.action',
											dataType : 'json',
											data : {
												"mobile" : $("#mobile").val(),
												"id" : id
											...
											success : function(data) {
												userTable.bootstrapTable(('refresh'));
												Modal.alert({ msg:'绑定钉钉成功！', title:'提示', btnok:'确定' }).on(function(e){
												});
											}
										});
										noticeUser = "";
		...
		function findNoticeUser(id){
			var noticeUser = "";
			$.ajax({
				type : 'post',
				url : 'noticeUserAction!findNoticeById.action',
				dataType : 'json',
				async: false,
				traditional :true, 
				data : {"id" : id},
		...
		function findByMobile(){
			...
			$.ajax({
				type : 'post',
				url : 'noticeUserAction!findNoticeByMobile.action',
				dataType : 'json',
				async: false,
				traditional :true, 
				data : {"mobile" : mobile},
